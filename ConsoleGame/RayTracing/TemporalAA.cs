...
public sealed class TemporalAA
{
    ...
    public TemporalAA(int width, int height, float taaAlpha = 0.05f, float motionTransReset = 0.0025f, float motionRotReset = 0.0025f)
    {
        ...
        history = new Fast2D<Vec3>(width, height);
        historyValid = false;
    }
    ...
    public Fast2D<Vec3> BlendIntoHistory(Fast2D<Vec3> current, bool forceReset = false, float? overrideAlpha = null)
    {
        if (current == null) throw new ArgumentNullException(nameof(current));
        if (current.Width != width || current.Height != height) throw new ArgumentException("Current buffer size does not match TAA history.");
